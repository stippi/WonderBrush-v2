...
// gaussian_blur
status_t
ImageProcessingLibrary::gaussian_blur(BBitmap *bitmap, float radius)
{
	...
	for (int32 i=-kernel_radius;i<=kernel_radius;i++) {
		kernel[i] /= sum;
		fixed_kernel[i] = (int32)(kernel[i] * 32768);
	}
...
fprintf(stderr, "ImageProcessingLibrary::gaussian_blur(): not enough memory to create temporary bitmap!\n");
	...
	for (int32 y = 0; y < height; y++) {
		...
		convolve_1d(source_array + kernel_radius,
					target_array,
					width,
					kernel,
					kernel_radius);

/*		convolve_1d_fixed(source_array + kernel_radius,
						  target_array,
						  width,
						  fixed_kernel,
						  kernel_radius);*/
...
// gaussian_blur
status_t
ImageProcessingLibrary::gaussian_blur(BBitmap *bitmap,float radius, int32 threadCount)
{
	int32 kernel_radius = (int32)ceil(radius);
	float *kernel_array = new float[2*kernel_radius+1];
	float *kernel = &kernel_array[kernel_radius];
...
fprintf(stderr, "ImageProcessingLibrary::gaussian_blur(): not enough memory to create temporary bitmap!\n");
	...
	// Start the threads.
	thread_id blur_thread_array[4]; // TODO: get actual number of CPUs
	for (int32 i=0;i<threadCount;i++) {
		int32 height = (int32)(bitmap_bounds.Height() / threadCount + 1);
		filter_thread_data *data = new filter_thread_data();
		data->left = 0;
		data->right = (int32)bitmap_bounds.right;
		int32 top;
		top = data->top = (int32)min_c(bitmap_bounds.bottom,bitmap_bounds.top + i*height);
		data->bottom = (int32)min_c(bitmap_bounds.bottom,top+height-1);
	...
	// Start the threads.
	for (int32 i=0;i<threadCount;i++) {
		int32 height = (int32)(intermediate_bounds.Height() / threadCount + 1);
		filter_thread_data *data = new filter_thread_data();
		data->left = 0;
		data->right = (int32)intermediate_bounds.right;
		int32 top;
		top = data->top = (int32)min_c(intermediate_bounds.bottom,intermediate_bounds.top + i*height);
		data->bottom = (int32)min_c(intermediate_bounds.bottom,top+height-1);
...
// convolve_1d
void
ImageProcessingLibrary::convolve_1d(uint32* s, uint32* t,
									int32 length,
									float* kernel,
									int32 kernel_radius)
{
...
colorSum += kernel[index];
}
			alpha += c.bytes[3] * kernel[index];
alphaSum += kernel[index];
		}

//		c.bytes[0] = (uint8)blue;
//		c.bytes[1] = (uint8)green;
//		c.bytes[2] = (uint8)red;
if (colorSum > 0.0) {
c.bytes[0] = (uint8)(blue / colorSum);
c.bytes[1] = (uint8)(green / colorSum);
c.bytes[2] = (uint8)(red / colorSum);
} else {
c.bytes[0] = 0;
c.bytes[1] = 0;
c.bytes[2] = 0;
...
if (alphaSum > 0.0)
		c.bytes[3] = (uint8)(alpha / alphaSum);
else
c.bytes[3] = 0;
...
// convolve_1d_fixed
void
ImageProcessingLibrary::convolve_1d_fixed(uint32* s, uint32* t,
										  int32 length,
										  int32* kernel,
										  int32 kernel_radius)
{
...
{
	int32 right,bottom;
	right = (int32)bitmap->Bounds().right;
	bottom = (int32)bitmap->Bounds().bottom;
	...
	for (int32 y=0;y<=bottom;y++) {
		...
		for (int32 x=0;x<=right;x++) {
			...
			map_value = (uint8)(left_coeff*(top_coeff*prev_histograms[current_bin-1][hist_index] +
...
void ImageProcessingLibrary::calculate_local_mapping_function(BBitmap *bitmap,int32 cx, int32 cy,int32 regionSize,uint8 *mapFunction)
{
	...
	left = cx;
	top = cy;
	right = (int32)min_c(left+regionSize-1,bitmap->Bounds().right);
	bottom = (int32)min_c(top+regionSize-1,bitmap->Bounds().bottom);
	...
	for (int32 i=0;i<256;i++) {
		mapFunction[i] = (uchar)floor(histogram[i]*multiplier);
	}
}
