/* 
	...
	Author:	Heikki Suhonen, additions by Stephan Aßmus
...
inline	int32	IntegerUniformDistribution(int32 a,int32 b);
		...
		void	SkipIntegers(uint32 numbers)
					{ integer_uniform_stream_position += numbers; }
		void	SkipFloats(uint32 numbers)
					{ uniform_stream_position += numbers; }
};
...
{
	if (random_array_length > 0) {
		return a + (b-a)*float_random_number_array[uniform_stream_position++ % random_array_length];
	}
	else {
