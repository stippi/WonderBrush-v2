...
// constructor
SimpleSelectState::SimpleSelectState(CanvasView* parent)
	: CanvasViewState(parent),
	  ...
	  fSelectAllLayers(false)
{
	fPrecise = false;
}

// destructor
SimpleSelectState::~SimpleSelectState()
{
...
// Init
void
SimpleSelectState::Init(Canvas* canvas, Layer* layer, BPoint currentMousePos)
{
...
// MouseDown
void
SimpleSelectState::MouseDown(BPoint where, Point canvasWhere, bool eraser)
{
...
// MouseUp
void
SimpleSelectState::MouseUp(BPoint where, Point canvasWhere)
{
...
// MouseMoved
void
SimpleSelectState::MouseMoved(BPoint where, Point canvasWhere,
...
// Draw
void
SimpleSelectState::Draw(BView* into, BRect updateRect)
{
...
void
SimpleSelectState::ModifyClippingRegion(BView* view, BRegion* region)
{
...
// MessageReceived
bool
SimpleSelectState::MessageReceived(BMessage* message)
{
	bool handled = true;
	switch (message->what) {
		case MSG_SELECTING: {
		...
		case MSG_SELECT_ALL_LAYERS: {
			int32 value;
			if (message->FindInt32("be:value", &value) >= B_OK)
				fSelectAllLayers = value == B_CONTROL_ON;
			break;
		}
		case MSG_SELECT_ENTIRE_CANVAS:
...
// ModifiersChanged
void
SimpleSelectState::ModifiersChanged()
{
	fShiftDown = modifiers() & B_SHIFT_KEY;
	if (fMouseDown) {
...
// HandleKeyDown
bool
SimpleSelectState::HandleKeyDown(uint32 key, uint32 modifiers)
{
	bool handled = true;
	switch (key) {
		case B_RETURN:
...
// UpdateToolCursor
void
SimpleSelectState::UpdateToolCursor()
{
...
// _SetRect
void
SimpleSelectState::_SetRect(BRect newBounds, bool action, bool notify)
{
	if (!fPrecise) {
	...
	if (action) {
		fCanvasView->Perform(new ChangeSelectRectAction(this, oldCropRect));
	}
	if (notify) {
		// send message
		if (BWindow* window = fCanvasView->Window()) {
			BMessage message(MSG_SELECTING);
...
// _Perform
void
SimpleSelectState::_Perform(BRect newBounds)
{
	...
	if (fSelectAllLayers) {
		if (fCanvas) {
			BBitmap* canvasBitmap = fCanvas->Bitmap();
			bitmap = cropped_bitmap(canvasBitmap, newBounds, false, true);
			delete canvasBitmap;
		}
	} else {
		if (fLayer) {
			bitmap = cropped_bitmap(fLayer->Bitmap(), newBounds, false, true);
		}
	}
	fCanvasView->HandleDemoMode(bitmap, newBounds);

	// copy the bitmap to the system clipboard
	if (bitmap && be_clipboard->Lock()) {
		if (BMessage* data = be_clipboard->Data()) {
			if (data && be_clipboard->Clear() >= B_OK) {
				BMessage bitmapArchive;
				if (bitmap && bitmap->Archive(&bitmapArchive) >= B_OK) {
					if (data->AddMessage("image/bitmap", &bitmapArchive) >= B_OK) {
		...
		// report error (probably tried to copy empty area)
		LanguageManager* manager = LanguageManager::Default();
		BAlert* alert = new BAlert("empty",
								   manager->GetString(SELECTED_AREA_EMPTY, "The selected area was empty."),
...
void
SimpleSelectState::_SetConfirmationEnabled(bool enable) const
{
...
// _DrawDragPoint
void
SimpleSelectState::_DrawDragPoint(BView* into, BRect r) const
{
...
void
SimpleSelectState::_InvalidateCanvasView(BRect r) const
{
	r = _ViewRect(r);
	r.InsetBy(-6.0, -6.0);
...
// _ViewRect
BRect
SimpleSelectState::_ViewRect(BRect r) const
{
...
// _DragModeFor
uint32
SimpleSelectState::_DragModeFor(BPoint where)
{
	if (!fCropRect.IsValid())
...
// _SetDragMode
void
SimpleSelectState::_SetDragMode(uint32 mode)
{
	if (mode != fDraggingMode) {
		fDraggingMode = mode;
		UpdateToolCursor();
